...
import { RootState, setContents } from 'store/store'
...
interface MDWriterProps {
    pdata: any;
}
const MDWriter = ({pdata}: MDWriterProps) => {
    const contents = useSelector( (state: RootState) => state.post.contents);
    const dispatch = useDispatch();

    const updateContents = (val: string) => {
        dispatch(setContents(val));
    }

    const setOverflow = (fullScreen: boolean) => {
        if(fullScreen){
    ...
    const delay = 1000;
    const optionsWriteMode: any = useMemo(() => {
        return {
        autofocus: false,
        spellChecker: false,
        lineWrapping : false,
        ...
        onToggleFullScreen: function(fullScreen: boolean) {
    ...
    const optionsEditMode: any = useMemo(() => {
        return {
        autofocus: false,
        spellChecker: false,
        lineWrapping : true,
    ...
    useEffect(() => {
        if(pdata === null){
            // write mode
            const autosavedValue: any = localStorage.getItem(`smde_autoSaving`);
