...
export default function Article({data}: DataProps){
    const {title,reg_date,hashtag,contents} = data;
    
    const { sideBarState, mobileFlag } = useSelector((state: RootState) => ({
    ...
    const PushSearchword = (txt: string) => {
        if(mobileFlag){
            dispatch(openSearchDialog());
            setTimeout(function() {
                const searchbox = document.getElementById('searchbox__mobile') as HTMLInputElement;
                searchbox.value = txt;
