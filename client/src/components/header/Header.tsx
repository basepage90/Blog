...
const HideHeader = () => {
    ...
    if (prevScrollpos > currentScrollPos || prevScrollpos === 0) {
        (document.getElementById("rootHeader") as HTMLInputElement).style.top = "0";
    } else {
        let height = (document.getElementById("rootHeader") as HTMLInputElement).offsetHeight;
        (document.getElementById("rootHeader") as HTMLInputElement).style.top = -height+"px";
    }
...
function Header({ loading, data }: HeaderProps){
    ...
    // Suject
    const makeSubject = () => {
        const locationArr = (location.pathname as any).split( '/', '3' )
        ...
        if(!loading){
            data.categoryList.map((lg: { category_lg: string; category_md: any[]; screen_name: string; }) => {
