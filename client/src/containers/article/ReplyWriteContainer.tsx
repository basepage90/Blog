...
interface ReplyWriterContainerProps {
  replyId: number|null;
  depth?: number;
  siblingName?: string;
  groupNo?: number;
...
const ReplyWriterContainer = ({replyId,depth,siblingName,groupNo,refetch,setOpenReply}: ReplyWriterContainerProps) =>{
    const { admin_flag, nickname } = useSelector(
      ...
      const replyValidator = (name:string, password:string ,contents: string) => {
        if (admin_flag){
      ...
      const handleSnackbaraVariant = (variant: VariantType, rv: string) => {
        let msg;
        switch (variant) {
          case 'success':
            msg = '댓글을 등록했어요 :)'
      ...
      const doCreateReply = () => {
        let name;
        let password;
        const contents = (document.getElementById('reply__contents__'+replyId) as HTMLInputElement).value;
        ...
        if(admin_flag){
          ...
          name = (document.getElementById('reply__name__'+replyId) as HTMLInputElement).value;
          password = (document.getElementById('reply__password__'+replyId) as HTMLInputElement).value;
        }
        ...
        const success = createReply({ variables: {
          ...
          success.then(({data})=> {
              ...
              if(data.createReply.id !== undefined || data.createReply.id !== null) {
                  ...
                  (document.getElementById('reply__contents__'+replyId) as HTMLInputElement).value = "";
