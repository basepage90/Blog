...
interface ReplyListContainerProps {
    replyList: any;
    refetch: any;
}

const ReplyListContainer = ({replyList,refetch}: ReplyListContainerProps) => {
    ...
    const handleClick = (e: { currentTarget: React.SetStateAction<null>; }, id: React.SetStateAction<number>) => {
        setAnchorEl(e.currentTarget);
        setCurrentRemoveId(id)
    };
    

    const ShowReplyChild = (replyId: React.SetStateAction<number>) => {
    ...
    const RenderReplyChild = (replyId: number | null, groupNo: number | undefined, replyName: string | undefined) => {
        return (
            <>
                {replyId === currentId &&
                    <ReplyChild >
    ...
    const doRemoveReply = () => {
        const password = (document.getElementById('pop__password') as HTMLInputElement).value;
